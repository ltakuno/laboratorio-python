    ...
    const promises = exercicioAtual.testes.map(teste => {
        ...
        return Sk.misceval.asyncToPromise(() => Sk.importMainWithBody("<stdin>", false, codigoParaExecutar, true))
            .then(() => {
                ...
                row.insertCell(0).innerHTML = `<pre style="font-size: 1.1em;">${teste.entrada}</pre>`;
                row.insertCell(1).innerHTML = `<pre style="font-size: 1.1em;">${esperado}</pre>`;
                row.insertCell(2).innerHTML = `<pre style="font-size: 1.1em;">${obtido}</pre>`;
